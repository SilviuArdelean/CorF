...
operations_manager::operations_manager(ustring const& xmlDocPath)
    : str_xml_file_path_(xmlDocPath),
      ptr_root_(nullptr),
      ptr_latest_namedop_(nullptr),
      ptr_latest_operation_(nullptr),
      ptr_latest_operation_list_(nullptr) {}

operations_manager::~operations_manager() {
  xml_tree_cleaner(ptr_root_);
}
...
bool operations_manager::loadXMLdocument() {
  if (!doc_xml_.load_file(str_xml_file_path_.c_str())) {
    cout << "XML document load process has failed. Cannot continue..."
         << std::endl;
    return false;
  }

  pugi::xml_node root = doc_xml_.child("namedoperations");

  return true;
}

xml_node* operations_manager::object_factory(ustring const& _tagstr,
                                             ustring const& _name,
                                             ustring const& _value) {
  if (string_utils::compare_nocase_sensitive(_tagstr, "namedoperations")) {
    ptr_root_ = new xml_namedoperations(nullptr, _name, _value);

    return ptr_root_;  // special case - root
  } else if (string_utils::compare_nocase_sensitive(_tagstr,
                                                    "namedoperation")) {
    ptr_latest_namedop_ = new xml_namedop(ptr_root_, _name, _value);

    return ptr_latest_namedop_;
  } else if (string_utils::compare_nocase_sensitive(_tagstr, "operation")) {
    if (ptr_latest_operation_list_ &&
        string_utils::compare_nocase_sensitive(str_latest_parent_name_,
                                               "operationlist")) {
      ptr_latest_operation_ =
          new xml_operation(ptr_latest_operation_list_, _name, _value);
      xml_operationlist* ptr_op_list =
          dynamic_cast<xml_operationlist*>(ptr_latest_operation_list_);
      if (!ptr_op_list)
        return nullptr;
      ptr_op_list->add_ops(ptr_latest_operation_);
    } else {
      ptr_latest_operation_ =
          new xml_operation(ptr_latest_namedop_, _name, _value);
    }

    return ptr_latest_operation_;
  } else if (string_utils::compare_nocase_sensitive(_tagstr, "returnvalue")) {
    return new xml_returnvalue(ptr_latest_namedop_, _name, _value);
  } else if (string_utils::compare_nocase_sensitive(_tagstr, "callnamedop")) {
    return new xml_callednamedop(ptr_latest_operation_, _name, _value);
    ;
  } else if (string_utils::compare_nocase_sensitive(_tagstr, "operationlist")) {
    ptr_latest_operation_list_ =
        new xml_operationlist(ptr_latest_operation_, _name, _value);
    return ptr_latest_operation_list_;
  }

  // OTHERS param - withparam - localvar, etc -> others
  { return new xml_others(ptr_latest_namedop_, _name, _value); }
};

void operations_manager::tree_builder(pugi::xml_document const& doc,
                                      pugi::xml_node node,
                                      int depth) {
  if (depth == 1)  // root's children
  {
    ...
    ptr_root_ = object_factory("namedoperations", "namedoperations", "");

    // tag::namedoperation[]
    for (pugi::xml_node _node = root.first_child(); _node;
         _node = _node.next_sibling()) {
      ustring tagName(_node.name());
      xml_node* xmlnode = object_factory(tagName, _node.name(), _node.value());

      for (pugi::xml_attribute attr = _node.first_attribute(); attr;
           attr = attr.next_attribute()) {
        xmlnode->add_attribute(attr.name(), attr.value());
      }

      ptr_root_->add(xmlnode);

      tree_builder(doc, _node, ++depth);
    }
    // end::namedoperation[]
  } else {
    for (pugi::xml_node _node = node.first_child(); _node;
         _node = _node.next_sibling()) {
      ustring tagName(_node.name());
      str_latest_parent_name_ = _node.parent().name();
...
void operations_manager::build_tree_xml_based(BUILD_TREE_MODE mode) {
  ...
  tree_builder(doc_xml_, root, depth);
}

void operations_manager::test_print_tree() {
  print_tree(ptr_root_);
}
...
void operations_manager::print_tree(xml_node* node, bool first_time) {
  ...
  if (first_time)
    cout << "#: name='" << node->get_tagname() << "', value='"
         << node->get_value() << "'\n";  // element
...
size_t operations_manager::run_get_namedoperation_count() {
  ...
  std::list<xml_node*> list_children;
  ptr_root_->get_children_list(list_children);

  for (auto& ob : list_children) {
    if (string_utils::compare_nocase_sensitive(pattern_namedops,
                                               ob->get_tagname())) {
      namedops_counter++;
    }
  }
  ...
  if (namedops_counter)
    cout << "There are " << namedops_counter << " '" << pattern_namedops << "'"
         << std::endl;
  else
...
// Which one is the least complex ? 	Complexity is measured by the number of
//operations inside each namedop.
void operations_manager::run_get_most_complex_namedop() {
  cout << "---------------------------------------------------------------"
       << std::endl;
  cout << "Q2 - Which namedop identified by it’s 'name' attribute is the most "
  ...
  for (auto& ob_namedop : list_children) {
    if (string_utils::compare_nocase_sensitive(strNamedOperation,
                                               ob_namedop->get_tagname())) {
      int ops_count = 0;
  ...
  while (true) {
    cout << "The most complex 'namedop' is '" << it_most->get_node_valuename()
         << "' (count " << most_counts_val << ")" << std::endl;
    ++it_most;
  ...
  while (true) {
    cout << "The less complex 'namedop' is '" << it_less->get_node_valuename()
         << "' (count " << less_counts_val << ")" << std::endl;
...
void operations_manager::build_nameop_complexities(
    xml_node* node,
  ...
  // add the complexities info within a priority_queue to easy extract max and
  // min comlexity
  const std::string strNamedOperation("namedoperation");
...
void operations_manager::count_operations(xml_node* node, int& ops_count) {
  ...
  for (auto& ob : list_children) {
    if (string_utils::compare_nocase_sensitive(strOperation,
                                               ob->get_tagname())) {
      ops_count++;
      ...
      for (auto& sub_op : ob_list_children) {
        if (string_utils::compare_nocase_sensitive(sub_op->get_tagname(),
                                                   strOperationList)) {
          ...
          {
            if (string_utils::compare_nocase_sensitive(
                    strOperation, op_list->get_tagname())) {
              ops_count++;
            }
          }
        }
...
void operations_manager::run_find_namedop_with_returnvalue() {
  ...
  find_namedop_returnvalue(ptr_root_, list_namedop_retvalue);
...
// Q3 - Which namedop has a return value ?
void operations_manager::find_namedop_returnvalue(
    xml_node* node,
  ...
  for (auto& ob_namedop : list_children) {
    if (string_utils::compare_nocase_sensitive(strNamedOperation,
      ...
      for (auto& ob_retval : list_named_children) {
        if (string_utils::compare_nocase_sensitive(ob_retval->get_tagname(),
                                                   strReturnValue)) {
...
void operations_manager::run_find_maximum_call_depth() {
  ...
  // update xml_callednamedop with the 'xml_namedop' name =
  // "namedop_to_be_executed" information
  update_callnamedop();
  ...
  xml_namedoperations* namedops = dynamic_cast<xml_namedoperations*>(ptr_root_);
  ...
  for (auto max_namedop : list_maxdepth) {
    cout << "namedoperation: " << max_namedop->get_node_valuename()
         << std::endl;
  }
}
...
void operations_manager::update_callnamedop() {
  ...
  ptr_root_->get_children_list(list_root_children);
...
void operations_manager::search_update_callnamed(xml_node* node) {
  ...
  if (string_utils::compare_nocase_sensitive(node->get_tagname(),
                                             "callnamedop")) {
    ...
    // update the called 'nameop' information that has to be available while
    // effective executing
