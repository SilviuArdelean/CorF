...
class fixed_queue : public std::priority_queue<_Ty, _Container, _Pr> {
 public:
  ...
  auto cbegin() -> decltype(_PQ_specialization::c.cbegin()) {
    return _PQ_specialization::c.cbegin();
  }

  auto end() -> decltype(_PQ_specialization::c.end()) {
    return _PQ_specialization::c.end();
  }
  auto cend() -> decltype(_PQ_specialization::c.cend()) {
    return _PQ_specialization::c.cend();
  }
